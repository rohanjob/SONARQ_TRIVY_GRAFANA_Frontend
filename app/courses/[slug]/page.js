...
export default function CourseDetailPage({ params }) {
  ...
  async function loadCourse() {
    try {
      ...
      setCourse(fallbackCourses.find((c) => c.slug === slug) || null);
    }
    setLoading(false);
  }
            ...
            <div className={styles['detail-header-inner']}>
              ...
              <div>
                ...
                <div className={styles['detail-meta']}>
                  <span className={`${styles['detail-meta-item']} ${styles['detail-rating']}`}>
                    ⭐ {Number(course.rating).toFixed(1)} ({Number(course.reviews_count).toLocaleString()} reviews)
                  </span>
              ...
              <div className={styles['purchase-card']}>
                ...
                <div className={styles['purchase-price']}>
                  <span className={styles['purchase-price-current']}>₹{Number(displayPrice).toLocaleString()}</span>
                  {course.discount_price && (
                    <span className={styles['purchase-price-original']}>₹{Number(course.price).toLocaleString()}</span>
                  )}
                </div>
                {discountPct > 0 && (
                  <div className={styles['purchase-discount']}>🎉 {discountPct}% off — Limited time offer!</div>
                )}
                ...
                <ul className={styles['purchase-features']}>
                  <li><span>✅</span> {course.duration_hours} hours on-demand video</li>
                  <li><span>✅</span> {course.lessons_count} downloadable resources</li>
